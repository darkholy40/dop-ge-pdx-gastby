...
const UserForm = ({ modification, id }) => {
  ...
  const savePageView = useCallback(() => {
    ...
    if (
      token !== `` &&
      userInfo._id !== `` &&
      roleNames[userInfo.role.name].level < 3
    ) {
      client(token).mutate({
        mutation: gql`
          mutation CreateLog {
  ...
  const getUser = useCallback(async () => {
    ...
    try {
      ...
      dispatch({
        ...
        notificationDialog: {
          open: true,
          title: `ไม่สามารถเชื่อมต่อฐานข้อมูลได้ getUser`,
      ...
      <Grid container spacing={2} sx={{ marginBottom: `1rem` }}>
        <Grid item xs={12}>
          <Flex>
            ...
            />
            <CheckCircleFlex>{renderCheckingIcon(inputs.rank)}</CheckCircleFlex>
          </Flex>
        </Grid>
      ...
      <Grid container spacing={2} sx={{ marginBottom: `1rem` }}>
        ...
        <Grid item xs={12}>
          <Flex>
            ...
            />
            <CheckCircleFlex>{renderCheckingIcon(inputs.role)}</CheckCircleFlex>
          </Flex>
        </Grid>
      </Grid>
      <Divider style={{ margin: `1rem auto 2rem`, width: 360 }} />
        ...
        <Grid item md={6} sm={12} xs={12}>
          {!modification ? (
            ...
            <Button
              ...
              onClick={() => getUser()}
            >
              <FontAwesomeIcon icon={faRedoAlt} style={{ marginRight: 5 }} />
              โหลดข้อมูลใหม่
            </Button>
          )}
        </Grid>
