...
import EducationNamesDialog from "../../components/databases/education-names-dialog"
...
const EducationNames = () => {
        ...
        mutation: gql`
          mutation CreateLog {
            createLog(input: {
              data: {
                action: "view",
                description: "databases->education-names",
  ...
  const getDataFromDB = useCallback(async () => {
    ...
    try {
      ...
      setTableOption(prev => ({
        ...prev,
        totalRows: total.data.educationNamesConnection.aggregate.count,
      }))

      if (total.data.educationNamesConnection.aggregate.count > 0) {
        ...
        let count = 0
        for (let row of res.data.educationNames) {
          count++
          returnData = [
            ...returnData,
            {
