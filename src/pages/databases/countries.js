...
import CountriesDialog from "../../components/databases/countries-dialog"
...
const Countries = () => {
        ...
        mutation: gql`
          mutation CreateLog {
            createLog(input: {
              data: {
                action: "view",
                description: "databases->countries",
  ...
  const getDataFromDB = useCallback(async () => {
    ...
    try {
      ...
      setTableOption(prev => ({
        ...prev,
        totalRows: total.data.countriesConnection.aggregate.count,
      }))

      if (total.data.countriesConnection.aggregate.count > 0) {
        ...
        let count = 0
        for (let row of res.data.countries) {
          count++
          returnData = [
            ...returnData,
            {
            ...
            <>
              ...
              <OparatorFlex>
                <div className="ft">
                  ...
                  >
                    <Chip
                      label={`ชื่อประเทศ: ${confirmedFilterInputs.name}`}
                      color="primary"
